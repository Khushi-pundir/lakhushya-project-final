...
async function geocodeRegistrationAddress({ address, city, state, pincode, country }) {
  ...
  throw new Error("Could not find this registration address on the map. Please enter a clearer address.");
}
...
                try {
                  const normalizedState = stateVal.replace(/UttarPradesh/gi, "Uttar Pradesh").trim();
                  const location = await geocodeRegistrationAddress({
                    address,
                    city,
